/* 루틴 트래커 — _data/days/*.yml 을 읽어 통계·차트를 그립니다. 외부 라이브러리 없음. */
(function () {
  "use strict";
  const RAW = window.TRACKER_DATA || {};
  ...
  const fmtMin = m => m == null ? "–" : `${pad(Math.floor(m / 60))}:${pad(Math.round(m % 60))}`;
  ...
  function normalize(k, raw) {
    ...
    const arrive = toMin(raw.arrive), leave = toMin(raw.leave), wake = toMin(raw.wake);
    let hours = raw.hours != null ? +raw.hours : null;
    if (arrive != null && leave != null && leave > arrive) hours = (leave - arrive) / 60;
    return {
  ...
  Object.keys(RAW).forEach(k => {
    if (/^\d{4}-\d{2}-\d{2}$/.test(k) && RAW[k] && typeof RAW[k] === "object") E[k] = normalize(k, RAW[k]);
  });
  const KEYS = Object.keys(E).sort();
  const get = k => E[k] || null;

  // ───────── 통계 ─────────
  function streak(pred) {               // 오늘(또는 어제)부터 거꾸로 연속 일수
    let d = TODAY, n = 0;
    if (!pred(get(keyOf(d)))) d = addDays(d, -1);
    while (pred(get(keyOf(d)))) { n++; d = addDays(d, -1); if (n > 5000) break; }
    return n;
  }
  function bestStreak(pred) {
    ...
    KEYS.forEach(k => {
      const d = parseKey(k);
      const consecutive = prev && (d - prev) / 86400000 === 1;
      if (pred(E[k])) cur = consecutive && cur > 0 ? cur + 1 : 1; else cur = 0;
      best = Math.max(best, cur); prev = d;
    });
    return best;
  }
  function weekRange(d = TODAY) {        // 월요일 ~ 일요일
  ...
  function renderToday(root) {
    ...
    const dayLabel = `${TODAY.getMonth() + 1}월 ${TODAY.getDate()}일 (${WD[TODAY.getDay()]})`;
    let html = `<span class="muted">${dayLabel}</span>`;
    if (!t) {
  ...
  function renderStats(root) {
    ...
    const [wa] = weekRange();
    const elapsed = Math.round((TODAY - wa) / 86400000) + 1;
    const moArr = mo.filter(e => e.arrive != null);
    const onTime = moArr.filter(e => e.arrive <= GOAL).length;
    const tiles = [];
    tiles.push(tile("📝 기록 스트릭", `${streak(logged)}<span class="unit">일</span>`, `최고 ${bestStreak(logged)}일 · 이번 주 ${wk.length}/${elapsed}일`));
    tiles.push(tile("🏢 이번 달 평균 출근", fmtMin(avg(moArr.map(e => e.arrive))), moArr.length ? `목표 ${fmtMin(GOAL)} 이내 ${onTime}/${moArr.length}일` : "기록 없음"));
    tiles.push(tile("⏱ 이번 달 평균 체류", `${fmtNum(avg(mo.map(e => e.hours)))}<span class="unit">시간</span>`, `평균 퇴근 ${fmtMin(avg(mo.map(e => e.leave)))}`));
    const sleepAvg = avg(mo.map(e => e.sleep)), moodAvg = avg(mo.map(e => e.mood)), focusAvg = avg(mo.map(e => e.focus));
    tiles.push(tile("🌙 수면 · 컨디션 · 집중", `${fmtNum(sleepAvg)}<span class="unit">h</span> · ${moodStr(moodAvg)} · ${fmtNum(focusAvg)}<span class="unit">h</span>`, "이번 달 평균"));
    root.innerHTML = `<div class="grid grid-4">${tiles.join("")}</div>`;
    const hb = HABITS.map(h => {
      const p = hasHabit(h.key);
      const w = wk.filter(p).length, m = mo.filter(p).length;
      const rate = mo.length ? Math.round(100 * m / mo.length) : 0;
      return tile(`${h.emoji} ${esc(h.label)}`, `${streak(p)}<span class="unit">일 연속</span>`, `이번 주 ${w}/${elapsed} · 이번 달 ${rate}% · 최고 ${bestStreak(p)}일`);
    });
    if (hb.length) root.insertAdjacentHTML("beforeend", `<div class="grid grid-4" style="margin-top:.9rem">${hb.join("")}</div>`);
    function tile(label, value, sub) { return `<div class="stat"><div class="stat-label">${label}</div><div class="stat-value">${value}</div><div class="stat-sub">${sub}</div></div>`; }
  }

  // ───────── 히트맵 ─────────
  function renderHeatmap(root, selectRoot) {
    let metric = "all";
    const weeks = 26, cell = 12, gap = 3, step = cell + gap, left = 24, top = 18;
    const level = e => {
      if (!e) return 0;
    ...
    function draw() {
      ...
      const svg = svgEl("svg", { class: "heatmap", width: left + weeks * step, height: top + 7 * step + 4, viewBox: `0 0 ${left + weeks * step} ${top + 7 * step + 4}` });
      [["월", 0], ["수", 2], ["금", 4]].forEach(([t, r]) => { const x = svgEl("text", { x: 0, y: top + r * step + cell - 2 }); x.textContent = t; svg.appendChild(x); });
      let lastMonth = -1;
      for (let c = 0; c < weeks; c++) {
        const mon = addDays(start, c * 7);
        if (mon.getMonth() !== lastMonth) {
          if (c === 0 ? addDays(mon, 6).getMonth() === mon.getMonth() : true) {
            const t = svgEl("text", { x: left + c * step, y: 10 }); t.textContent = `${mon.getMonth() + 1}월`; svg.appendChild(t);
          }
        ...
        for (let r = 0; r < 7; r++) {
          ...
          const rect = svgEl("rect", { x: left + c * step, y: top + r * step, width: cell, height: cell, class: `heat-${level(e)}${k === TODAY_KEY ? " heat-today" : ""}` });
      ...
      root.innerHTML = "";
      const wrap = el("div", { class: "heatmap-wrap" }); wrap.appendChild(svg); root.appendChild(wrap);
    ...
    if (selectRoot) {
      ...
      selectRoot.querySelectorAll("button").forEach(b => b.addEventListener("click", () => {
        metric = b.dataset.v; selectRoot.querySelectorAll("button").forEach(x => x.classList.toggle("on", x === b)); draw();
      }));
    }
    draw();
  }
  ...
  function renderArriveChart(root) {
    const pts = KEYS.map(k => E[k]).filter(e => e.arrive != null).slice(-30);
    if (pts.length < 2) { root.innerHTML = `<div class="empty">출근 시간 기록이 2일 이상 쌓이면 추이 차트가 표시됩니다.</div>`; return; }
    ...
    pts.forEach((p, i) => { if (i % every === 0 || i === pts.length - 1) { const t = svgEl("text", { x: x(i), y: H - 8, "text-anchor": "middle" }); t.textContent = `${p.date.getMonth() + 1}/${p.date.getDate()}`; axis.appendChild(t); } });
    svg.appendChild(axis);
    svg.appendChild(svgEl("line", { x1: L, x2: W - R, y1: y(GOAL), y2: y(GOAL), class: "goal" }));
    const gl = svgEl("text", { x: W - R, y: y(GOAL) - 4, "text-anchor": "end", class: "axis" }); gl.textContent = `목표 ${fmtMin(GOAL)}`; gl.style.fill = "var(--warn)"; gl.style.fontSize = "10px"; svg.appendChild(gl);
    const path = pts.map((p, i) => `${i ? "L" : "M"}${x(i).toFixed(1)},${y(p.arrive).toFixed(1)}`).join(" ");
    svg.appendChild(svgEl("path", { d: `${path} L${x(pts.length - 1).toFixed(1)},${H - B} L${L},${H - B} Z`, class: "area" }));
    svg.appendChild(svgEl("path", { d: path, class: "line" }));
    pts.forEach((p, i) => {
      const c = svgEl("circle", { cx: x(i), cy: y(p.arrive), r: 3.5, class: `dot${p.arrive > GOAL ? " late" : ""}` });
    ...
    const a = avg(vals), late = vals.filter(v => v > GOAL).length;
    root.insertAdjacentHTML("beforeend", `<div class="small muted">최근 ${pts.length}일 평균 출근 <b>${fmtMin(a)}</b> · 목표 초과 ${late}일</div>`);
  }
  ...
  function renderRecent(root, days = 14) {
    ...
    for (let i = 0; i < days; i++) {
      ...
      rows.push(`<tr style="cursor:pointer" data-k="${k}">
        <td>${dl}</td>
        <td class="num ${e.arrive != null && e.arrive > GOAL ? "late" : ""}">${fmtMin(e.arrive)}</td>
        <td class="num hide-sm">${fmtMin(e.leave)}</td>
        <td class="num hide-sm">${e.hours != null ? fmtNum(e.hours) + "h" : "–"}</td>
        <td class="emojis">${hab}</td>
        <td class="mood hide-sm">${moodStr(e.mood)}</td>
        <td class="num hide-sm">${e.sleep != null ? fmtNum(e.sleep) + "h" : "–"}</td>
  ...
  function initForm(root) {
    ...
    root.innerHTML = `
      ...
      <p class="small muted">저장 버튼은 GitHub의 새 파일 화면을 내용이 채워진 상태로 엽니다. <b>Commit changes</b>를 누르면 1~2분 뒤 이 페이지에 반영됩니다.
      터미널에서는 <code>python scripts/log.py --arrive 9:10 english coding</code> 처럼 기록할 수도 있습니다.</p>`;
    ...
    root.querySelector("#tr-save").addEventListener("click", () => {
      ...
      const exists = !!get(d);
      const url = exists
        ? `https://github.com/${REPO}/edit/${BRANCH}/_data/days/${d}.yml`
        : `https://github.com/${REPO}/new/${BRANCH}/_data/days?filename=${encodeURIComponent(d + ".yml")}&value=${encodeURIComponent(yaml())}`;
      if (exists) navigator.clipboard?.writeText(yaml()).catch(() => {});
      window.open(url, "_blank", "noopener");
      if (exists) flash("이미 기록이 있는 날이라 편집 화면을 엽니다. YAML은 클립보드에 복사했으니 붙여넣기 하세요.");
    });
    ...
    function update() {
      ...
      const d = val("date");
      root.querySelector("#tr-save").textContent = get(d) ? "GitHub에서 수정 ↗" : "GitHub에 저장 ↗";
      root.querySelector("#tr-hint").textContent = `→ _data/days/${d || "YYYY-MM-DD"}.yml`;
    }
    function flash(msg) { const h = root.querySelector("#tr-hint"); h.textContent = msg; setTimeout(update, 4000); }
    root._update = update;
  }
  ...
  function initExport(btn) {
    btn.addEventListener("click", () => {
      ...
      const rows = KEYS.map(k => { const e = E[k]; return [k, WD[e.date.getDay()], fmtMin(e.arrive), fmtMin(e.leave), e.hours != null ? fmtNum(e.hours, 2) : "", ...HKEYS.map(h => e.done.has(h) ? 1 : 0), fmtMin(e.wake), e.sleep ?? "", e.mood ?? "", e.focus ?? "", e.note].map(v => `"${String(v).replace(/"/g, '""').replace(/–/g, "")}"`).join(","); });
